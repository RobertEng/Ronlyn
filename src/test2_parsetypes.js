const TokenType = { // used for type definition
...
const TokenTag = {  // used as comparators
  ATSIGN: '@',
...
const NumberTokenPattern = /([0-9]+)/g;
const PunctuationTokenPattern = /([,.\/#$%\^&\*;:{}=\-_'~()\"\?\.!@])/g;
const MLstartTokenPattern = /(<[\w\s="/.':;#-\/\?]+>)/g;
const MLendTokenPattern = /(<\/[\w\s="/.':;#-\/\?]+>)/g;
*/
...
/*
const AppSpecificTagToTypeMap = [
  { tag: TokenTag.CONTRACTION, type: TokenType.CONTRACTION },
  { tag: TokenTag.NUMBER_WITHCOMMAS, type: TokenType.NUMBER_WITHCOMMAS }
];
*/
/*
const MLtagType = [
  { tag: TokenTag.USD,
    parseType: ParseType.USD,
    pattern: /(?<=^|\W)\$(([1-9]\d{0,2}(,\d{3})*)|(([1-9]\d*)?\d))(\.\d\d)?(?=\s|\W|$|[.!?\\-])/g },
  ...
  { tag: TokenTag.CONTRACTION,
    parseType: ParseType.CONTRACTION,
    pattern: /(?<=^|\s|^$)([A-Za-z]+)\'d(?=\s|\W|$|[.!?\\-])/g },
  ...
  { tag: TokenTag.CONTRACTION,
    parseType: ParseType.CONTRACTION, // actually mostlly possessive
...
/*
ParseDispatchMap = [
  { tag: TokenTag.CONTRACTION, parseType:"" }
]
*/
module.exports = { TokenType, ParserNodeType, endMarkupTag , MarkupTokenTypeList, TokenizerTokenTypeList,
                  TokenTag };
